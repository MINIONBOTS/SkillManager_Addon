-- Power2 check condition
local sm_condition_power = class('Power2', sm_condition)
sm_condition_power.uid = "Power2"
...
function sm_condition_power:Evaluate(skill,context)
	if ( context.player) then
		local mp = context.player.power2
...
function sm_condition_power:Render(id) -- need to pass an index value here, for the unique IDs used by imgui	
	...
	GUI:InputText("##sm_condition_power21"..tostring(id), GetString("Player Power2 Value"),GUI.InputTextFlags_ReadOnly)
	if (GUI:IsItemHovered()) then GUI:SetTooltip("This Power 2 is for Necro Sand Shades or Thief Malice.") end
	GUI:PopItemWidth()
	
	GUI:PushItemWidth(50)
	GUI:SameLine()
	self.operator, changed = GUI:Combo("##sm_condition_power22"..tostring(id),self.operator, self.operators)
	if ( changed ) then modified = true end
	GUI:PopItemWidth()
	
	GUI:PushItemWidth(100)
	GUI:SameLine()
	self.value, changed = GUI:InputInt("##sm_condition_power23"..tostring(id),self.value, 1,10,GUI.InputTextFlags_CharsDecimal+GUI.InputTextFlags_CharsNoBlank)
