...
function sm_condition_distance:Evaluate(skill,context)
	-- Use:
	-- context.player
	-- context.attack_target	(== "Enemy")
	-- context.heal_target	(== "Friend")
	-- We are ignoring this distance check if this is a comboskill, this way the first skill in the combo defines the range when it starts, and all others can individually be cast if they are in range
	if(skill.temp.iscombo)then
		return true
	end
	...
	if ( t ) then
		local pos,bb = SkillManager:PredictedPositionAndDistance(t)
		local radius = t.radius

		if ( self.operator == 1 ) then return (bb - radius) < self.value
		elseif ( self.operator == 2 ) then return (bb - radius) <= self.value
		elseif ( self.operator == 3 ) then return (bb - radius) == self.value
		elseif ( self.operator == 4 ) then return (bb - radius) >= self.value
		elseif ( self.operator == 5 ) then return (bb - radius) > self.value
		end
	end
	return false
end
