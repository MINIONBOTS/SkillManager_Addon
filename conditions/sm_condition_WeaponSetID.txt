...
local sm_condition_endurance = class('WeaponSet ID', sm_condition)
sm_condition_endurance.uid = "WeaponSet ID"
...
function sm_condition_endurance:Evaluate(skill,context)
	return context.player and context.player.weaponset	 == self.value
end
...
function sm_condition_endurance:Render(id) -- need to pass an index value here, for the unique IDs used by imgui	
	local modified
	local changed
	GUI:Text(GetString("Weapon Set ID is "))
	...
	if ( self.value > 10 ) then self.value = 10 end
